...
function uploadPaths(source, name) {
  const match = source.match(/actions\/upload-artifact@v7[\s\S]*?path:\s*\|\n((?: {12}.+\n)+)\s+if-no-files-found:/);
  assert.ok(match, `${name} must declare explicit upload paths`);
...
function assertReusableWorkflow(source, name) {
  ...
  assert.equal((source.match(/^permissions:/gm) ?? []).length, 1, `${name} must declare permissions exactly once`);
...
test('Android workflow builds and verifies the explicit release evidence tree', async () => {
  ...
  for (const [, script] of source.matchAll(/run:\s*\|((?:\n {10,}.*)*)/g)) {
    assert.doesNotMatch(script, /\$\{\{ secrets\./, 'Android run scripts must not interpolate secrets');
  }
...
test('web extension workflow uses the exact Node reference and publishes only package files', async () => {
  ...
  assert.deepEqual(uploadPaths(source, 'build-web-extension.yml'), [
    'build/web-extension/veles-extension-*.zip',
    'build/web-extension/veles-extension-*.zip.sha256',
    'build/web-extension/SHA256SUMS',
  ], 'Web workflow must upload exactly the three package files');
...
test('supply-chain workflow enforces reports before uploading exactly three SBOMs', async () => {
  ...
  assert.deepEqual(uploadPaths(source, 'build-supply-chain.yml'), [
  ...
  ], 'Supply-chain workflow must upload only named SBOM and enforcement report files');
