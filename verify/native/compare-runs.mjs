...
import { error, mismatch, parseNativeManifest, sha256 } from '../lib/checksum-manifest.mjs';
...
function verifyProductManifest(run) {
  ...
  const actual = new Map(productEntries(run).map((entry) => [entry.path.slice(8), entry]));
...
export async function compareRuns(resolvedCommit, leftPath, rightPath, output) {
  ...
  const leftAll = new Map([...productEntries(left).map((entry) => [`product/${entry.path.slice(8)}`, entry]), ...[...leftView].map(([path, entry]) => [`view/${path}`, entry])]);
  const rightAll = new Map([...productEntries(right).map((entry) => [`product/${entry.path.slice(8)}`, entry]), ...[...rightView].map(([path, entry]) => [`view/${path}`, entry])]);
